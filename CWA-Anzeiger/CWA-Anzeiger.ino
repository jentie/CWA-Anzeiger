...
int oldCWA = 0;                   // CWA-Wert aus dem letzten Scanzyklus
    ...
    void onResult(BLEAdvertisedDevice advertisedDevice) {       // BLE Device gefunden
      if (advertisedDevice.haveServiceUUID()) {
        ...
        if (service.equals(BLEUUID((uint16_t)0xFD6F))) {        // UUID der Corona-Warn-App
          ...
          if (numCWA > oldCWA)                                  // zusätzliche CWA melden
...
void setup() {
  ...
  for (int i = 0; i < 3; i++) {                     // bei Start blinkt LEDs und Anzeige
    digitalWrite(LEDint, HIGH);
    digitalWrite(LEDext, HIGH);
...
void loop() {                                       // ewige Wiederholung des Scan-Zyklus
  ...
  if (numCWA > maxCWA)                              // ggf. als Höchstwert speichern
    maxCWA = numCWA;
  oldCWA = numCWA;                                  // Zähler des aktuellen Scanzyklus merken
  ...
  digitalWrite(LEDint, LOW);                        // CWA-Indikator vor nächstem Scan ausschalten
  digitalWrite(LEDext, LOW);                        // Indikator "zusätzliche CWA" ausschalten
}
